...
static inline int cp14_is_enabled(void) {
    uint32_t ret = cp14_dscr_get();
    ret = (ret & (1 << 15)) && (~(ret & (1 << 14)));
    return ret;
}
...
static inline void cp14_bcr0_disable(void) {
    cp14_bcr0_set(cp14_bcr0_get() & ~0b1);
    prefetch_flush();
}
...
static inline int was_brkpt_fault(void) {
    ...
    if (bits_get(cp15_ifsr_get(), 0, 3) == 0b0010) {
        ret = bits_get(cp14_dscr_get(), 2, 5) == 0b0001;
    }
    return ret;
}
...
static inline int was_watchpt_fault(void) {
    ...
    if (bits_get(cp15_dfsr_get(), 0, 3) == 0b0010) {
        ret = bits_get(cp14_dscr_get(), 2, 5) == 0b0010;
    }
    return ret;
